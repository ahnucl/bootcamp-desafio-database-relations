...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    if (foundProducts.length !== products.length) {
      throw new AppError("Some products aren't registered.");
    }

    const productsWithInsuficientQuantities = foundProducts.filter(
    ...
    const productsInOrder = foundProducts.map((product, index) => ({
